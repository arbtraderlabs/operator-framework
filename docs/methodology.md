...
request
  │
  ▼
preflight        (skills/preflight.md)   — confirm scope, access, safety before doing anything
  │
  ▼
resolve          (skills/resolve.md)     — turn an ambiguous ask into a clear, agreed problem statement
  │
  ▼
write-spec       (skills/write-spec.md)  — produce a written specification of the solution
  │
  ▼
plan-tickets     (skills/plan-tickets.md)— decompose the spec into small, orderable tickets
  │
  ▼
execute-ticket   (skills/execute-ticket.md) — do the work for one ticket at a time
  │
  ▼
report           (skills/report.md)      — summarize what was executed, against the ticket/spec
  │
  ▼
verify           (skills/verify.md)      — independent check that the work meets the spec (human gate)
  │
  ▼
handoff          (skills/handoff.md)     — persist state so work can pause/resume cleanly
  │
  ▼
public-release   (skills/public-release.md) — final scan and sign-off before anything goes public
```

`execute-ticket` → `report` → `verify` repeats per ticket until the ticket
backlog for the spec is empty (see [`docs/ticket-system.md`](ticket-system.md)).
...
| Decision ledger entries | any skill, as decisions are made | [`templates/decision-ledger-entry.md`](../templates/decision-ledger-entry.md) |
...
[`docs/model-routing.md`](model-routing.md) for a routing framework you can
...
- See [`skills/public-release.md`](../skills/public-release.md) for the
