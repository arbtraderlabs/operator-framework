...
3. **Check the environment only to the depth the ticket needs.** If the ticket
   has an **Environment** block or the work has operational / public / real-world
   side effects, compare the expected target and workspace responsibility with
   the observed state. Continue when they align or the difference is understood
   and contained; warn when something looks inconsistent; stop before a clear
   consequential or explicitly forbidden action. See
   [`docs/ENVIRONMENT_GUARDRAILS.md`](../../docs/ENVIRONMENT_GUARDRAILS.md).
4. **Execute within scope**, honouring any **Route** overrides on the ticket
...
5. **Stop rather than silently expanding scope or improvising.** If the
...
6. **Do not introduce real proprietary, private, or secret data** at any
   point, even temporarily (see
   [`README.md#security-and-privacy`](../../README.md#security-and-privacy)).
7. **Log any material decisions** (approach choices, trade-offs) in the
...
- Any environment warning that remains relevant to release or verification.
...
independent [`skills/verify/skill.md`](../verify/skill.md). A scope blocker or
clear consequential environment conflict is a stop-and-classify checkpoint
